...
Важно упомянуть, что к большинству исполняемых файлов приведена соответсвующая документация и гибкая настройка параметров, вызвать её можно при например так:
```nushell
...
Логика сохранения чекпоинтов заключается в:s
