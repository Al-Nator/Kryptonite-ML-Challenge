...
<ol type="I">
  ...
  <li>
    ...
    <ul>
      ...
      <li><a href="#clean-dataset">Очистка датасета</a></li>
...
<a id="results"></a>
## 🏆 Результаты
### Команда MMG заняла 🥇 место и получила приз 300 т.р. победив сразу в 2-х номинациях!

![vk_results_0](https://github.com/user-attachments/assets/d5073752-1df1-41e7-a7e3-7d4d4df09802)

![vk_results_1](https://github.com/user-attachments/assets/9bb209bf-0bf9-486c-9a5f-cae636d34f75)
...
В нём описаны основные моменты по всей работе и наши умозаключения.
...
│   ├── align.py <------------------------ Скрипт для предобработки изображений (alignment, resize)
...
├── notebooks
│   ├── embedding_visualization.ipynb <---- Визуализация эмбедингов
│   ├── model_attack.ipynb <-------------- Атаки на модель
├── web <--------------------------------- Веб-интерфейс
...
В проекте используется Anaconda.
...
Отлично! Когда пары сгенерировались, можно приступать к обучению.
...
Данных аргументов будет достаточно чтобы запустить обучение на очищенных данных.
...
А теперь о самой очистке датасета.
...
Далее запускаем скрипт для очистки датасета:
```
